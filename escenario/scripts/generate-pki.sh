...
elif [[ -x "$BIN_DIR/scion-pki" ]]; then
  SCION_PKI="$BIN_DIR/scion-pki"
...
CORE_ISSUER["1-110"]="1-110"
CORE_ISSUER["1-111"]="1-111"
CORE_ISSUER["1-112"]="1-110"
CORE_ISSUER["1-113"]="1-111"
CORE_ISSUER["1-114"]="1-110"

CORE_ISSUER["2-210"]="2-210"
CORE_ISSUER["2-211"]="2-211"
CORE_ISSUER["2-212"]="2-210"
CORE_ISSUER["2-213"]="2-211"
CORE_ISSUER["2-214"]="2-210"

CORE_ISSUER["3-310"]="3-310"
CORE_ISSUER["3-311"]="3-311"
CORE_ISSUER["3-312"]="3-310"
CORE_ISSUER["3-313"]="3-311"
CORE_ISSUER["3-314"]="3-310"

CORE_ISSUER["4-410"]="4-410"
CORE_ISSUER["4-411"]="4-411"
CORE_ISSUER["4-412"]="4-410"
CORE_ISSUER["4-413"]="4-411"
CORE_ISSUER["4-414"]="4-410"
...
clean_node_crypto() {
  ...
  rm -f "$node_dir"/config/*.db || true
  rm -f "$node_dir"/config/database/*.db || true
  rm -f "$node_dir"/config/database/*.db-shm || true
  rm -f "$node_dir"/config/database/*.db-wal || true
  rm -f "$node_dir"/config/tmp/* || true
  rm -f "$node_dir"/config/logs/* || true
  ...
  ensure_dir "$node_dir/config/keys"
  ensure_dir "$node_dir/config/database"
  ensure_dir "$node_dir/config/tmp"
  ensure_dir "$node_dir/config/logs"
}
...
write_time_env() {
  ...
  not_before="$(date -u -d '-24 hours' +%Y-%m-%dT%H:%M:%SZ)"
  ...
  as_not_after="$(date -u -d '+180 days' +%Y-%m-%dT%H:%M:%SZ)"
...
copy_runtime_ca_layout() {
  ...
  copy_to_node "$root_dir/cp-root.pem" "$node_dir/config/certs/cp-root.pem"
  copy_to_node "$ca_dir/cp-ca.pem" "$node_dir/config/certs/cp-ca.pem"
  copy_to_node "$ca_dir/cp-ca.pem" "$node_dir/config/certs/ISD${isd}-AS${asnum}.ca.crt"
}
...
generate_core_bundle() {
  ...
  copy_to_node "$as_dir/chain.pem" "$node_dir/config/certs/ISD${isd}-AS${asnum}.pem"
  copy_to_node "$as_dir/cp-as.key" "$node_dir/config/crypto/as/cp-as.key"

  copy_runtime_ca_layout "$isd" "$asnum" "$node_dir" "$root_dir" "$ca_dir"
...
generate_leaf_as_cert() {
  ...
  copy_to_node "$out_dir/chain.pem" "$node_dir/config/certs/ISD${isd}-AS${asnum}.pem"
  ...
  copy_to_node "$issuer_ca_dir/cp-ca.pem" "$node_dir/config/crypto/ca/ISD${isd}-AS${issuer_num}.ca.crt"

  copy_to_node "$issuer_root_dir/cp-root.pem" "$node_dir/config/certs/cp-root.pem"
  copy_to_node "$issuer_ca_dir/cp-ca.pem" "$node_dir/config/certs/cp-ca.pem"
}
