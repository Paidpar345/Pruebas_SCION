...
[ca]
server = 3000
clients = ["123=config/client1.secret"]
EOF
}
...
leaf_link_port() {
  ...
  local leafidx="$2"
  local coreidx="$3"
  echo $((51000 + isd * 100 + leafidx * 10 + coreidx))
}
...
write_core_topology_json() {
  ...
  cat > "$tmp" <<EOF
{
  "attributes": ["authoritative", "issuing", "voting", "core"],
  "isd_as": "$asid",
  "mtu": 1500,
  "control_service": {
    "cs-1": { "addr": "$ip:30254" }
...
EOF
  ...
  coreidx=0
  [[ "$asid" == "${ISD_CORE_B[$isd]}" ]] && coreidx=1
  for leaf in ${ISD_LEAFS[$isd]}; do
    leafip="${AS_IP[$leaf]}"
    leafidx="$(leaf_index_in_isd "$leaf")"
    port="$(leaf_link_port "$isd" "$leafidx" "$coreidx")"
    if [[ $first -eq 0 ]]; then
      echo "," >> "$tmp"
    fi
    first=0
...
write_leaf_topology_json() {
  ...
  local corea="${ISD_CORE_A[$isd]}"
  local coreb="${ISD_CORE_B[$isd]}"
  local ipa="${AS_IP[$corea]}"
  local ipb="${AS_IP[$coreb]}"
  local leafidx
  leafidx="$(leaf_index_in_isd "$asid")"
  local porta
  porta="$(leaf_link_port "$isd" "$leafidx" 0)"
  local portb
  portb="$(leaf_link_port "$isd" "$leafidx" 1)"
...
write_compose_service() {
  ...
  local ip="$5"
  local rel="${node_dir#$SCENARIO_DIR/}"

  cat >> "$compose_file" <<EOF
  $service_name:
    image: ubuntu:22.10
    container_name: $container_name
    command: ["bash", "-lc", "cd /root/escenario/$rel && ./run.sh"]
    cap_add:
      - ALL
    privileged: true
    volumes:
